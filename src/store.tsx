...
export const StoreProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  // Restore cached announcement and media from client IndexedDB immediately on mount (zero localStorage)
  useEffect(() => {
    ...
    idbGet<MediaType[]>('sarkar_custom_media').then(cachedMedia => {
      if (Array.isArray(cachedMedia) && cachedMedia.length > 0) {
        setMediaItems(deduplicateMedia(cachedMedia));
      }
    });
  }, []);
  ...
  const addMedia = async (media: any, onProgress?: (percent: number) => void) => {
    ...
    const updatedList = [newMedia, ...mediaItems.filter(m => m.id !== tempId && m.url !== media.url)];
    setMediaItems(updatedList);

    try {
      await idbSet('sarkar_custom_media', updatedList);
    } catch (err) {
      console.warn('Media IDB cache save note:', err);
    }
  ...
  const updateMedia = async (id: string, updatedData: any, onProgress?: (percent: number) => void) => {
    ...
    try {
      ...
      console.warn('Media IDB cache update note:', err);
    }
    if (onProgress) onProgress(100);
  };
  ...
  const deleteMedia = async (id: string | number) => {
    ...
    try {
      await idbSet('sarkar_custom_media', updated);
    } catch (err) {
      console.warn('Media IDB delete note:', err);
    }
  };
  ...
  const clearAllMedia = async () => {
    setMediaItems([]);
    try {
      await idbSet('sarkar_custom_media', []);
    } catch (err) {}
  };
