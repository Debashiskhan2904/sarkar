...
// Active purge helper: completely wipe all keys from localStorage and IndexedDB so nothing is cached locally
const purgeLegacyLocalStorage = () => {
  ...
  // Also clear any persistent IndexedDB cache to guarantee pure real-time cloud data
  idbClear().catch(() => {});
};
...
export const StoreProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const addMedia = async (media: any, onProgress?: (percent: number) => void) => {
    ...
    try {
      ...
      if (docRef?.id) {
        setMediaItems(prev => prev.map(m => m.id === tempId ? { ...m, id: docRef.id } : m));
