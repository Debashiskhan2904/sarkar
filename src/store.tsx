...
export const StoreProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const unsubJobs = onSnapshot(collection(db, 'jobs'), async (snapshot) => {
      setDbConnected(true);
      console.log(`[Firestore:Jobs] Snapshot received with ${snapshot.docs.length} document(s) from 'jobs' collection.`);
      ...
      if (snapshot.empty && !seedingJobs) {
        ...
        try {
          ...
          if (!isExplicitlyCleared) {
            console.log('[Firestore:Jobs] Seeding default company openings into fresh Firestore database...');
            ...
            console.log('[Firestore:Jobs] Jobs collection is empty and explicitly cleared by admin.');
            setJobs([]);
          }
        } catch (e) {
          console.warn('[Firestore:Jobs] Job auto-seed note:', e);
        } finally {
          seedingJobs = false;
        }
        return;
      ...
      const activeData = data.filter(j => {
        const jId = String(j.id);
        const jTitle = (j.title || '').trim().toLowerCase();
        if (deletedJobsRef.current.has(jId)) return false;
        if (jTitle && (deletedJobsRef.current.has('title:' + jTitle) || deletedJobsRef.current.has('preset:' + jTitle))) return false;
        return true;
      });

      console.log(`[Firestore:Jobs] Active jobs loaded: ${activeData.length} (out of ${data.length} raw documents).`, activeData.map(j => ({ id: j.id, title: j.title, category: j.category, creator: j.creatorEmail || j.createdBy || 'General' })));
      ...
      setJobs(deduplicateJobs(activeData));
    }, (err) => {
      console.error('[Firestore:Jobs] Realtime snapshot listener error:', err.message);
    });
  ...
  const addJob = async (job: any) => {
    ...
    const newJob: JobType = { 
      ...
      desc: (job.desc || '').trim(),
      creatorEmail: (job.creatorEmail || '').trim() || undefined,
      createdBy: (job.createdBy || '').trim() || undefined,
      createdAt: job.createdAt || new Date().toISOString(),
      id: tempId
    };

    console.log('[Firestore:Jobs] Posting new job:', { title: newJob.title, category: newJob.category, creator: newJob.creatorEmail || newJob.createdBy || 'Admin' });
    ...
    if (job.title) {
      ...
      deletedJobsRef.current.delete(tempId);
      try {
    ...
    try {
      const sanitized = sanitizeForFirestore({
        ...
        desc: newJob.desc,
        creatorEmail: newJob.creatorEmail,
        createdBy: newJob.createdBy,
        createdAt: newJob.createdAt
      });
      const docRef = await addDoc(collection(db, 'jobs'), sanitized);
      if (docRef?.id) {
        console.log(`[Firestore:Jobs] Job persisted successfully with doc ID: ${docRef.id}`);
        setJobs(prev => prev.map(j => j.id === tempId ? { ...j, id: docRef.id } : j));
        return docRef.id;
      }
      return tempId;
    } catch (err: any) {
      console.error('[Firestore:Jobs] addDoc error:', err?.message);
      return tempId;
    }
  };
  ...
  const deleteJob = async (id: string | number) => {
    ...
    const targetJob = jobs.find(j => String(j.id) === strId);
    if (targetJob?.title) {
      deletedJobsRef.current.add('title:' + targetJob.title.trim().toLowerCase());
    }

    // Immediately remove from React state
    setJobs(prev => prev.filter(j => String(j.id) !== strId));

    // 1. Delete direct document by ID if real Firestore doc ID
    try {
    ...
    // 2. Also search Firestore by title to ensure any doc is deleted even if id was temp or local
    if (targetJob?.title) {
      try {
        const q = query(collection(db, 'jobs'), where('title', '==', targetJob.title.trim()));
        ...
        console.warn('Job query delete by title note:', err?.message);
      }
    }
