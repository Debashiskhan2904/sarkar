...
export const StoreProvider = ({ children }: { children: React.ReactNode }) => {
  const [jobs, setJobs] = useState<JobType[]>(() => {
    ...
    return DEFAULT_JOBS.filter(j => !isJobDeleted(j, deletedJobsSet));
  });
  ...
  const [mediaItems, setMediaItems] = useState<MediaType[]>(() => {
    ...
    if (isCleared) return [];
    return DEFAULT_MEDIA.filter(m => !isMediaDeleted(m, deletedMediaSet));
  });
  ...
  useEffect(() => {
    ...
    const unsubJobs = onSnapshot(collection(db, 'jobs'), (snapshot) => {
      ...
      const isInitialized = localStorage.getItem('sarkar_jobs_initialized');

      if (data.length === 0 && !isInitialized && !seedingJobs) {
        seedingJobs = true;
        localStorage.setItem('sarkar_jobs_initialized', 'true');
        const seedList = DEFAULT_JOBS.filter(j => !isJobDeleted(j, deletedJobsSet));
        seedList.forEach(j => {
          const { id, ...rest } = j as any;
          addDoc(collection(db, 'jobs'), rest).catch(err => console.log('Seeding jobs failed:', err.message));
        });
        setJobs(seedList);
        localStorage.setItem(SAVED_JOBS_KEY, JSON.stringify(seedList));
      } else {
    ...
    const unsubMedia = onSnapshot(collection(db, 'media'), async (snapshot) => {
      ...
      let finalMedia: MediaType[] = [];

      if (isCleared && mergedCustomDocs.length === 0) {
        finalMedia = [];
      } else {
        const existingTitles = new Set(mergedCustomDocs.map(d => (d.title || '').trim().toLowerCase()));
        const existingIds = new Set(mergedCustomDocs.map(d => String(d.id)));

        const availableDefaults = isCleared ? [] : DEFAULT_MEDIA
          .filter(m => !isMediaDeleted(m, deletedMediaSet))
          .filter(m => !existingIds.has(String(m.id)) && !existingTitles.has((m.title || '').trim().toLowerCase()));

        finalMedia = [...mergedCustomDocs, ...availableDefaults];
      }
  ...
  const clearMediaByType = async (type: string) => {
    ...
    DEFAULT_MEDIA.filter(m => m.type === type).forEach(m => {
      if (m.id !== undefined) deletedSet.add(String(m.id));
      if (m.title) deletedSet.add('title:' + m.title.trim().toLowerCase());
      if (m.url) deletedSet.add('url:' + m.url.trim());
    });
  ...
  const restoreDefaultMedia = async () => {
    ...
    try {
      localStorage.setItem(SAVED_MEDIA_KEY, JSON.stringify(DEFAULT_MEDIA));
    } catch {}
    await idbSet(SAVED_MEDIA_KEY, DEFAULT_MEDIA);
