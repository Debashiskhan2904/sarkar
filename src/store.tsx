...
export const StoreProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const unsubMedia = onSnapshot(collection(db, 'media'), (snapshot) => {
      ...
      if (isCleared) {
        finalMedia = activeFirestoreDocs;
      } else {
        const existingTitles = new Set(activeFirestoreDocs.map(d => (d.title || '').trim().toLowerCase()));
        const existingIds = new Set(activeFirestoreDocs.map(d => String(d.id)));

        const availableDefaults = DEFAULT_MEDIA
        ...
        finalMedia = [...activeFirestoreDocs, ...availableDefaults];
      }

      setMediaItems(finalMedia);
      localStorage.setItem(SAVED_MEDIA_KEY, JSON.stringify(finalMedia));
    }, (err) => {
      console.error('media listener error:', err.message);
      const deletedMediaSet = getDeletedSet(DELETED_MEDIA_KEY);
