...
import { saveMediaChunks, loadMediaChunks, deleteMediaChunks, isLargePayload } from './lib/mediaChunkStorage';
...
// Active purge helper: completely wipe all keys from localStorage so nothing ever remains in local storage
...
export const StoreProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  // Restore cached announcement from client IndexedDB immediately on mount (zero localStorage)
  useEffect(() => {
    idbGet<AnnouncementType>('sarkar_live_notice').then(cached => {
  ...
  useEffect(() => {
    // 1. Purge legacy business data from localStorage so none remains locally
    ...
    const unsubDeleted = onSnapshot(doc(db, 'settings', 'deleted_records'), (snap) => {
      ...
      if (snap.exists()) {
        ...
        if (typeof data.mediaCleared === 'boolean') {
          mediaClearedRef.current = data.mediaCleared;
          if (data.mediaCleared) {
            setMediaItems([]);
          }
        }
    ...
    // 4. Real-time Media from Firestore
    const unsubMedia = onSnapshot(collection(db, 'media'), (snapshot) => {
      setDbConnected(true);
      if (mediaClearedRef.current) {
        setMediaItems([]);
        return;
      }
      const firestoreDocs = snapshot.docs.map(d => ({ ...d.data(), id: d.id })) as MediaType[];
      const activeFirestoreDocs = firestoreDocs.filter(m => !isMediaDeleted(m, deletedMediaRef.current));

      setMediaItems(deduplicateMedia(activeFirestoreDocs));

      // Asynchronously resolve and populate any chunked media items (e.g. audio/video files)
      activeFirestoreDocs.forEach(async (m) => {
        if (m.isChunked && (!m.url || m.url === '')) {
          const loadedUrl = await loadMediaChunks(String(m.id));
          if (loadedUrl) {
            setMediaItems(prev => prev.map(item => String(item.id) === String(m.id) ? { ...item, url: loadedUrl } : item));
          }
        }
      });
    }, (err) => {
    ...
    // 5. Real-time Applications from Firestore
      ...
      if (noticeDoc) {
        try {
          ...
          if (parsed && typeof parsed.active === 'boolean') {
            ...
            idbSet('sarkar_live_notice', synced);
    ...
    const unsubAnnouncement = onSnapshot(doc(db, 'settings', 'announcement'), (snapshot) => {
      ...
      if (snapshot.exists()) {
        ...
        idbSet('sarkar_live_notice', syncedAnnouncement);
        setLastNoticeSyncTime(new Date().toLocaleTimeString());
      }
    }, () => {});
  ...
  const addMedia = async (media: any, onProgress?: (percent: number) => void) => {
    mediaClearedRef.current = false;

    if (media.title) deletedMediaRef.current.delete('title:' + media.title.trim().toLowerCase());
    if (media.url) deletedMediaRef.current.delete('url:' + media.url.trim());

    const fullDataUrl = typeof media.url === 'string' ? media.url : '';
    const needsChunking = isLargePayload(fullDataUrl);

    const tempId = 'med_' + Date.now();
    const newMedia = { ...media, id: tempId, url: fullDataUrl };

    setMediaItems(prev => [newMedia, ...prev.filter(m => m.id !== tempId && m.url !== media.url)]);
    ...
    try {
      const sanitized = sanitizeForFirestore({
        ...
        fileSize: fullDataUrl.length,
        url: needsChunking ? '' : fullDataUrl
      });
      delete sanitized.id;
      const docRef = await addDoc(collection(db, 'media'), sanitized);
      const assignedId = docRef?.id || tempId;

      if (needsChunking) {
        await saveMediaChunks(assignedId, fullDataUrl, onProgress);
      }

      setMediaItems(prev => prev.map(m => m.id === tempId ? { ...m, id: assignedId, url: fullDataUrl } : m));
      return assignedId;
    } catch (err: any) {
  ...
  const updateMedia = async (id: string, updatedData: any, onProgress?: (percent: number) => void) => {
    const fullDataUrl = typeof updatedData.url === 'string' ? updatedData.url : '';
    const needsChunking = isLargePayload(fullDataUrl);

    setMediaItems(prev => prev.map(m => String(m.id) === String(id) ? { ...m, ...updatedData } : m));
    try {
      const sanitized = sanitizeForFirestore({
        ...updatedData,
        updatedAt: new Date().toISOString(),
        ...(needsChunking ? { isChunked: true, fileSize: fullDataUrl.length, url: '' } : {})
      });
      if (sanitized) {
        delete sanitized.id;
      ...
      if (needsChunking) {
        await saveMediaChunks(String(id), fullDataUrl, onProgress);
      }
    } catch (err: any) {
  ...
  const deleteMedia = async (id: string | number) => {
    ...
    if (typeof id === 'number' && itemToDelete?.title) {
      deletedMediaRef.current.add('preset:' + itemToDelete.title.trim().toLowerCase());
    }

    const updated = mediaItems.filter(m => String(m.id) !== strId && !isMediaDeleted(m, deletedMediaRef.current));
    setMediaItems(updated);

    if (updated.length === 0) {
      mediaClearedRef.current = true;
    }

    try {
      await deleteDoc(doc(db, 'media', strId));
      await deleteMediaChunks(strId);
    } catch (err: any) {
    ...
    try {
      await setDoc(doc(db, 'settings', 'deleted_records'), {
        ...
        mediaCleared: mediaClearedRef.current,
        updatedAt: new Date().toISOString()
      }, { merge: true });
    } catch (err: any) {
  ...
  const clearAllMedia = async () => {
    mediaItems.forEach(m => {
      if (m.id !== undefined) deletedMediaRef.current.add(String(m.id));
      if (m.title) deletedMediaRef.current.add('title:' + m.title.trim().toLowerCase());
      if (m.url) deletedMediaRef.current.add('url:' + m.url.trim());
    });
    mediaClearedRef.current = true;
    setMediaItems([]);
  ...
  const restoreDefaultMedia = async () => {
    mediaClearedRef.current = false;
    deletedMediaRef.current = new Set<string>();
  ...
  const updateAnnouncement = async (data: Partial<AnnouncementType>) => {
    ...
    // 1. Immediately update local state and IndexedDB (zero delay client persistence, no localStorage)
    setAnnouncement(updated);
    await idbSet('sarkar_live_notice', updated);
    ...
    try {
      ...
      // Client is already persistently updated via IndexedDB
      return true;
    } finally {
    ...
    try {
      ...
      if (inqSnap.exists()) {
        ...
        if (parsed && typeof parsed.active === 'boolean') {
          ...
          setAnnouncement(synced);
          await idbSet('sarkar_live_notice', synced);
          setLastNoticeSyncTime(new Date().toLocaleTimeString());
          return synced;
        }
      }
      ...
      // 3. Fallback to client IndexedDB
      const cached = await idbGet<AnnouncementType>('sarkar_live_notice');
      if (cached) {
        setAnnouncement(cached);
        return cached;
      }
