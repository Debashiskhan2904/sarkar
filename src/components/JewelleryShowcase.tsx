...
export const JewelleryShowcase: React.FC<JewelleryShowcaseProps> = ({
  ...
  // Automatically default: 3rd scheme (index 2) and 5th scheme (index 4) open in Fit ('fit'), others open in Fill Grid ('fill')
  useEffect(() => {
    if (activeImageIndex === 2 || activeImageIndex === 4) {
      // 3rd scheme (M-1 Scheme 2022) and 5th scheme (M2 Scheme Recycling Process) open in fit mode
      setImageFitMode('fit');
    } else {
      // 1st, 2nd, and 4th schemes open in fill grid
  ...
  // Dynamic localization for the official Jewellery schemes
  ...
  return (
    <div id="jewellery-showcase-card" className="jewellery-banner-card bg-gradient-to-b from-[#0e1017] to-[#08090d] border border-amber-500/30 rounded-2xl overflow-hidden shadow-2xl relative mb-12">
      ...
      <div className="bg-gradient-to-r from-[#171305] via-[#231b08] to-[#120f04] px-4 sm:px-6 py-4 border-b border-amber-500/30 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {/* Jewellery Scheme Tab Buttons */}
        <div className="w-full md:w-auto overflow-x-auto no-scrollbar py-0.5">
          <div className="grid grid-cols-2 sm:grid-cols-3 xl:flex xl:items-center gap-1.5 p-1 bg-black/70 rounded-xl border border-white/10 w-full">
            {images.map((img, idx) => {
