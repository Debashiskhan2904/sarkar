...
export const Preloader = () => {
  ...
  useEffect(() => {
    // 1. Immediately kick off loading the hero globe poster image and video stream as soon as preloader starts
    const globePoster = new Image();
    globePoster.src = "https://images.pexels.com/videos/3129957/free-video-3129957.jpg";

    const preloadVid = document.createElement('video');
    preloadVid.src = "https://videos.pexels.com/video-files/3129957/3129957-hd_1920_1080_25fps.mp4";
    preloadVid.preload = "auto";
    preloadVid.muted = true;
    preloadVid.playsInline = true;
    preloadVid.load();

    // 2. Calibrated luxury preloader duration (~1.7s baseline)
    const duration = 1700; 
    ...
    const timer = setInterval(() => {
      ...
      if (currentStep >= steps) {
        // Verify if hero video is ready or if safety timeout reached
        const heroVideo = document.querySelector('.hero-video') as HTMLVideoElement | null;
        const isReady = !heroVideo || heroVideo.readyState >= 2 || currentStep >= (steps + 30); // max extra ~600ms safety limit
