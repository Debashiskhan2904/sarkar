...
export const AdminPanel = () => {
  ...
  const handleMediaFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploadFileSize(`${sizeMB.toFixed(2)} MB`);
                          ...
                          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                            {currentTabMedia.length > 0 && (
                              <motion.button
                                ...
                                type="button"
                                className="btn-sm btn-danger"
                                style={{ cursor: 'pointer', background: 'rgba(235, 77, 75, 0.2)', border: '1px solid #eb4d4b', color: '#ff7675', fontWeight: 600 }}
                                onClick={async () => {
                                  if (window.confirm(`Are you sure you want to delete ALL ${currentTabMedia.length} ${tabTitle} from the website and admin panel?`)) {
                                    await clearMediaByType(targetType);
                                    showToast(`All ${tabTitle} removed from website`, 'success');
                                  }
                                }}
                              >
                                🗑️ Delete All {tabTitle}
                              </motion.button>
                            )}
                            {mediaItems.length > 0 && (
                              <motion.button
                                ...
                                type="button"
                                className="btn-sm btn-danger"
                                style={{ cursor: 'pointer', background: '#d63031', color: '#fff', fontWeight: 600 }}
                                onClick={async () => {
                                  if (window.confirm(`Are you sure you want to delete ALL ${mediaItems.length} media assets (Photos, Videos, Audios, Certificates) across the entire website?`)) {
                                    await clearAllMedia();
                                    showToast('All media assets permanently removed from website', 'success');
                                  }
                                }}
                              >
                                ⚠️ Clear All Media
