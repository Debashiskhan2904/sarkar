...
export const AdminPanel = () => {
  ...
  const addMediaItem = async (type: string) => {
    ...
    try {
      ...
      showToast(`${type === 'credential' ? 'Certificate' : type.charAt(0).toUpperCase() + type.slice(1)} added and saved to database!`, 'success');
    } catch {
      showToast('Failed to save media item to database', 'error');
    }
  };
  ...
  const handleSaveAnnouncement = async () => {
    ...
    try {
      ...
      showToast(annActive ? 'Live Website Announcement updated & published for all devices!' : 'Banner saved as INACTIVE (Hidden from all devices)', 'success');
    } catch {
      showToast('Failed to sync announcement to server', 'error');
    }
  };
