...
export const AdminPanel = () => {
  ...
  const handleMediaFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 15 * 1024 * 1024) {
      ...
      showToast(`File size (${originalSizeMB.toFixed(2)} MB) exceeds 15 MB limit. Please compress or paste a direct streaming URL.`, 'error');
      return;
    }
    ...
    if (file.type.startsWith('image/')) {
      ...
      reader.onload = () => {
        ...
        setMUrl(dataUrl);
        setIsUploading(false);
        showToast(`File "${file.name}" loaded (${originalSizeMB.toFixed(2)} MB ready for chunked upload)`, 'success');
      };
      reader.onerror = () => {
        setIsUploading(false);
        showToast('Failed to read file from device', 'error');
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (error?.code === 'auth/unauthorized-domain') {
        showToast('Domain not authorized. Please add your domain to Firebase Console -> Authentication -> Settings -> Authorized domains.', 'error');
      } else if (error?.code === 'auth/popup-blocked') {
        showToast('Popup was blocked by your browser. Please allow popups for this site or open preview in a new tab.', 'error');
      } else if (error?.code === 'auth/cancelled-popup-request' || error?.code === 'auth/popup-closed-by-user') {
        // Benign cancellation by user or replaced by newer request - show subtle info without spamming error
        showToast('Sign-in cancelled or window closed.', 'info');
      } else if (error?.message?.includes('INTERNAL ASSERTION FAILED')) {
        showToast('Sign-in process reset. Please try clicking once to log in.', 'info');
      } else {
        showToast(`Login failed: ${error?.message || 'Authentication error'}`, 'error');
      }
    } finally {
  ...
  const addMediaItem = async (type: string) => {
    ...
    try {
      ...
      showToast(`${type === 'credential' ? 'Certificate' : type.charAt(0).toUpperCase() + type.slice(1)} uploaded and saved to database!`, 'success');
    } catch (err: any) {
      showToast(err?.message || 'Failed to save media item to database', 'error');
    } finally {
          ...
          >
            ...
            {!authed ? (
              ...
              >
                ...
                <p style={{ marginTop: '14px', fontSize: '0.78rem', color: 'rgba(255,255,255,0.45)', textAlign: 'center', lineHeight: 1.4 }}>
                  Tip: If using the preview iframe, ensure popups are allowed or open the app in a new tab if prompted.
                </p>
              </motion.div>
              </div>
            ) : (
                    ...
                    <div className="admin-section active">
                      ...
                      >
                        {[
                          ...
                          { num: mediaItems.filter((m: any) => m.type==='photo' && !String(m?.id || '').startsWith('jewel-img-') && !String(m?.id || '').startsWith('interior-img-')).length, lbl: 'Photos' },
                          { num: mediaItems.filter((m: any) => m.type==='video' && !String(m?.id || '').startsWith('jewel-img-') && !String(m?.id || '').startsWith('interior-img-')).length, lbl: 'Videos' },
                          { num: mediaItems.filter((m: any) => m.type==='audio' && !String(m?.id || '').startsWith('jewel-img-') && !String(m?.id || '').startsWith('interior-img-')).length, lbl: 'Audios' },
                          { num: mediaItems.filter((m: any) => m.type==='credential' && !String(m?.id || '').startsWith('jewel-img-') && !String(m?.id || '').startsWith('interior-img-')).length, lbl: 'Certificates' }
                        ].map((stat, sIdx) => (
                          <motion.div 
                            key={sIdx}
                            variants={{
                ...
                <div className="admin-section active">
                  ...
                  <div className="admin-form" style={{ marginBottom: '32px', background: '#111111', padding: 'clamp(14px, 4vw, 24px)', borderRadius: '16px', border: '1px solid rgba(255,215,0,0.25)', width: '100%', maxWidth: '100%', boxSizing: 'border-box' }}>
                    ...
                    {(activeTab === 'audios' || activeTab === 'videos') && (
                      <div style={{ background: 'rgba(255, 215, 0, 0.08)', border: '1px solid rgba(255, 215, 0, 0.3)', borderRadius: '10px', padding: '12px 14px', marginBottom: '16px', fontSize: '0.84rem', color: '#f0f0f0', lineHeight: 1.5 }}>
                        <strong style={{ color: '#ffd700' }}>⚡ High-Capacity Media Storage Active:</strong> 
                        <div style={{ marginTop: '4px' }}>
                          • <strong>Device Files (up to 15 MB):</strong> Select any MP3/WAV/MP4 file directly from your computer/mobile. Our automated chunking system splits large audio/video payloads and streams them seamlessly through Firestore and local caching.<br />
                          • <strong>Online Links (Unlimited):</strong> You can also paste any external web streaming URL (Google Drive, Dropbox, Cloudinary, Catbox, or direct MP3/MP4 link).
                        </div>
                      </div>
                    )}
                    ...
                    {isSavingMedia && (
                      <div style={{ marginBottom: '16px', background: 'rgba(0,0,0,0.4)', borderRadius: '8px', padding: '10px 14px', border: '1px solid rgba(255,215,0,0.3)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', color: '#ffd700', fontSize: '0.85rem', fontWeight: 600, marginBottom: '6px' }}>
                          <span>Saving & chunking media to Firestore database...</span>
                  ...
                  {(() => {
                    ...
                    const currentTabMedia = mediaItems.filter((m: any) => 
                      m.type === targetType && 
                      !String(m?.id || '').startsWith('jewel-img-') && 
                      !String(m?.id || '').startsWith('interior-img-')
                    );
