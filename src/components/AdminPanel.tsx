...
export const AdminPanel = () => {
  const { 
    ...
    mediaItems, addMedia, updateMedia, deleteMedia, clearAllMedia, clearMediaByType, restoreDefaultMedia, purgeFirestoreMedia,
  ...
  const currentAdminName = user?.displayName || currentAdminDetails?.name || 'B.P. Kishore';
    ...
    checkRedirectResult().then(redirectUser => {
      if (redirectUser) {
        ...
        if (authorizedEmail) {
          ...
          logout();
          const err = `Unauthorized account (${userEmails[0]}). Authorized administrator: B.P. Kishore (bpkishore2001@gmail.com)`;
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (authorizedEmail) {
        ...
        const msg = `Signed in with ${primary}, which is not an authorized administrator. Please choose bpkishore2001@gmail.com in the Google account chooser, or use the Admin Access Key below.`;
        setAuthErrorNotice(msg);
        if (recordAuditLog) {
          recordAuditLog(primary, 'Unauthorized Login Attempt', `Sign-in blocked for account: ${primary}`, 'auth');
        }
        showToast(`Unauthorized account (${primary}). Authorized administrator: bpkishore2001@gmail.com`, 'error');
      }
    } catch (error: any) {
  ...
  const handleSaveEditJob = async () => {
    ...
    try {
      ...
      showToast('Job updated and synced with database!', 'success');
    } catch (err: any) {
      showToast(err?.message || 'Failed to update job in database', 'error');
    } finally {
      setIsSavingJob(false);
    }
  };
  ...
  const handleDeleteJob = async (id: string) => { 
    try {
      ...
      showToast('Job opening removed from database and website', 'success'); 
    } catch (err: any) {
                ...
                <div className="admin-section active">
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
                    <div>
                      ...
                      <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.85rem', margin: '4px 0 0 0' }}>
                        Add, edit, or delete corporate career openings. Changes sync immediately to the database and live website.
                      </p>
                    </div>
                  ...
                  >
                      ...
                      <div>
                        ...
                        <p style={{ color: '#888', fontSize: '0.82rem', margin: '4px 0 0 0' }}>
                          {editingJob 
                            ...
                            : 'Enter the job specifications to publish an active opening directly to the database.'}
                        </p>
                      </div>
                    ...
                    <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
                      {editingJob ? (
                        <>
                          <motion.button 
                            whileHover={{ scale: isSavingJob ? 1 : 1.02 }}
                            whileTap={{ scale: isSavingJob ? 1 : 0.98 }}
                            className="btn btn-primary" 
                            disabled={isSavingJob}
                            onClick={handleSaveEditJob} 
                            style={{ padding: '12px 28px', fontWeight: 700, fontSize: '0.95rem', cursor: isSavingJob ? 'not-allowed' : 'pointer', background: '#ffd700', color: '#000', opacity: isSavingJob ? 0.7 : 1 }}
                          >
                            {isSavingJob ? '💾 Saving to Database...' : '💾 Save & Update Job Opening'}
                          </motion.button>
                          <motion.button 
                            whileHover={{ scale: 1.02 }}
                            whileTap={{ scale: 0.98 }}
                        ...
                        <motion.button 
                          ...
                          style={{ padding: '12px 28px', fontWeight: 700, fontSize: '0.95rem', cursor: isSavingJob ? 'not-allowed' : 'pointer', opacity: isSavingJob ? 0.7 : 1 }}
                        >
                          {isSavingJob ? '⏳ Syncing to Database...' : '+ Post Job Opening'}
                        </motion.button>
                      )}
                    </div>
                  </div>

                  {/* Jobs Management Table with Search & Filter */}
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px', flexWrap: 'wrap', gap: '12px' }}>
                      <div>
                        ...
                        <span style={{ fontSize: '0.8rem', color: '#aaa' }}>
                          Click <strong>Edit</strong> on any row to load into the form above, or <strong>Delete</strong> to immediately remove.
                        </span>
                      </div>
                          ...
                          {(() => {
                            const filteredJobs = jobs.filter((j: any) => {
                              ...
                              const matchesSearch = !q || 
                                ...
                                (j.loc && j.loc.toLowerCase().includes(q)) ||
                                (j.desc && j.desc.toLowerCase().includes(q));
                              const matchesCat = jobCategoryFilter === 'All' || (j.category || 'General') === jobCategoryFilter;
                            ...
                            if (filteredJobs.length === 0) {
                              return (
                                <tr>
                                  <td colSpan={6} style={{ textAlign: 'center', color: '#888', padding: '36px 16px' }}>
                                    {jobs.length === 0 ? (
                                      <div>
                                        <p style={{ margin: '0 0 6px 0', fontSize: '0.95rem', color: '#ccc' }}>No corporate job listings currently published.</p>
                                        <p style={{ margin: 0, fontSize: '0.82rem', color: '#888' }}>Use the form above to add and publish a new career opportunity.</p>
                                      </div>
                                    ) : (
                                      <div>
                                        <p style={{ margin: '0 0 8px 0' }}>No jobs match your search/filter criteria.</p>
                                        <button
                                          type="button"
                                          onClick={() => { setJobSearchQuery(''); setJobCategoryFilter('All'); }}
                                          style={{
