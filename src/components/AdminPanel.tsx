...
export const AdminPanel = () => {
  ...
  const handleMediaFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Security: Validate file binary magic bytes before accepting
    ...
    if (file.type.startsWith('image/')) {
      try {
        ...
        setUploadFileSize(`${originalSizeMB.toFixed(2)} MB (Ready for Supabase CDN)`);
        ...
        showToast(`Photo "${file.name}" verified and ready to publish`, 'success');
      } catch (err: any) {
    ...
    try {
      ...
      if (selectedMediaFile) {
        ...
        if (uploadRes.url) {
          ...
          console.warn('Supabase upload notice:', uploadRes.error);
        }
      }
      ...
      await addMedia({ 
        ...
        setUploadProgress(Math.max(80, pct));
      });
      ...
      if (mediaFileInputRef.current) mediaFileInputRef.current.value = '';
      if (thumbFileInputRef.current) thumbFileInputRef.current.value = '';
      showToast(`${type === 'credential' ? 'Certificate' : type.charAt(0).toUpperCase() + type.slice(1)} saved to Supabase CDN!`, 'success');
    } catch (err: any) {
