  ...
  if (isSubmitted) {
    return (
      <div style={{ background: '#0a1a0f', padding: '24px', borderRadius: '8px', border: '1px solid #10b981', textAlign: 'center' }}>
        ...
        <h4 style={{ color: '#10b981', fontSize: '1.2rem', marginBottom: '8px', fontWeight: 700 }}>{t('contactPayFormSuccess')}</h4>
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="form-row" style={{ marginBottom: '12px' }}>
        <div>
          <label style={{ display: 'block', color: 'rgba(255,255,255,0.7)', fontSize: '0.75rem', marginBottom: '4px', fontWeight: 600 }}>{t('contactPayFormNameLabel')}</label>
          <input 
            type="text" 
        ...
        <div>
          <label style={{ display: 'block', color: 'rgba(255,255,255,0.7)', fontSize: '0.75rem', marginBottom: '4px', fontWeight: 600 }}>{t('contactPayFormPhoneLabel')}</label>
          <input 
            type="tel" 
      ...
      <div className="form-row" style={{ marginBottom: '16px' }}>
        <div>
          <label style={{ display: 'block', color: 'rgba(255,255,255,0.7)', fontSize: '0.75rem', marginBottom: '4px', fontWeight: 600 }}>{t('contactPayFormUtrLabel')}</label>
          <input 
            type="text" 
        ...
        <div>
          <label style={{ display: 'block', color: 'rgba(255,255,255,0.7)', fontSize: '0.75rem', marginBottom: '4px', fontWeight: 600 }}>{t('contactPayFormAmountLabel')}</label>
          <input 
            type="number" 
      ...
      >
        {isSubmitting ? t('contactPayFormSubmitting') : t('contactPayFormSubmitBtn')}
      </button>
    </form>
  );
};
...
export const Modals = () => {
  ...
  return (
    <>
      ...
      {zoomModal && (() => {
        const activeTitle = zoomModal.titles?.[zoomModal.start] || zoomModal.title || 'Sarkaar Enterprise Catalog';
        return (
              ...
              <div className="brochure-frame">
                ...
                <div className="brochure-bar">
                  <div className="brochure-bar-left">
                    <span className="blabel">Sarkaar Enterprise · Catalog</span>
          ...
          <div className="modal-content" style={{ maxWidth: '700px', width: '100%', maxHeight: '90vh', background: '#111111', border: '1px solid rgba(255, 215, 0, 0.4)', borderRadius: '16px', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden', margin: 'auto', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.9)' }}>
            ...
            <div className="modal-body payment-modal-scroll" style={{ padding: '32px 24px', overflowY: 'auto', maxHeight: '100%', display: 'flex', flexDirection: 'column', gap: '24px' }}>
              <div style={{ textAlign: 'center' }}>
                <span style={{ color: '#FFD700', fontSize: '0.8rem', fontWeight: 800, letterSpacing: '0.15em', textTransform: 'uppercase' }}>{t('contactPayBadge')}</span>
                <h2 style={{ fontFamily: "'Playfair Display', serif", color: '#ffffff', fontSize: '1.8rem', marginTop: '6px' }}>{t('contactPayTitle')}</h2>
                <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.9rem', marginTop: '4px' }}>{t('contactPaySub')}</p>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(210px, 1fr))', gap: '20px', alignItems: 'center', background: '#080808', padding: '20px', borderRadius: '12px', border: '1px solid rgba(255, 255, 255, 0.08)' }}>
                ...
                <div>
                  <h4 style={{ color: '#FFD700', margin: '0 0 10px 0', fontSize: '1.1rem', fontWeight: 700 }}>{t('contactBankTitle')}</h4>
                  <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: '0.88rem', margin: '0 0 6px 0' }}><strong>{t('contactAccName')}</strong> SARKAAR ENTERPRISE</p>
                  <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: '0.88rem', margin: '0 0 6px 0' }}><strong>{t('contactBankName')}</strong> HDFC Bank</p>
                  <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: '0.88rem', margin: '0 0 6px 0' }}><strong>{t('contactAccNo')}</strong> 402918237129</p>
                  <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: '0.88rem', margin: '0 0 6px 0' }}><strong>{t('contactIfsc')}</strong> HDFC0001234</p>
                  <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: '0.88rem', margin: '0' }}><strong>{t('contactGst')}</strong> 19AAAAA0000A1Z5</p>
                </div>
              </div>

              <div style={{ background: '#161616', padding: '20px', borderRadius: '12px', border: '1px solid rgba(255, 215, 0, 0.25)' }}>
                <h4 style={{ color: '#ffffff', fontSize: '1rem', margin: '0 0 14px 0', fontWeight: 700 }}>{t('contactPayFormTitle')}</h4>
                <PaymentForm />
              </div>
            </div>
          </div>
